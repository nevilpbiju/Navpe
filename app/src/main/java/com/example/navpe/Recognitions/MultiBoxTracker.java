...
public class MultiBoxTracker {
    ...
    public synchronized void draw(final Canvas canvas) {
        ...
        for (final TrackedRecognition recognition : trackedObjects) {
            ...
            final String strConfidence = recognition.detectionConfidence < 0 ? "" : String.format("%.2f", (recognition.detectionConfidence)) + "";
            final String labelString = !TextUtils.isEmpty(recognition.title) ? String.format("Accuracy is %s",  strConfidence) : strConfidence;
    ...
    private void processResults(final List<SimilarityClassifier.Recognition> results) {
        ...
        for (final Pair<Float, SimilarityClassifier.Recognition> potential : rectsToTrack) {
            ...
            trackedRecognition.color = potential.second.getColor() != null ?  potential.second.getColor() : COLORS[trackedObjects.size()];
